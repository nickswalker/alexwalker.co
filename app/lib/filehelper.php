...
class Filehelper {
	...
	public static function get_dir_file_info($source_dir, $top_level_only = TRUE, $_recursion = FALSE)
	{
		...
		if ($fp = @opendir($source_dir))
		{
			...
			while (FALSE !== ($file = readdir($fp)))
			{
				...
				elseif (strncmp($file, '.', 1) !== 0)
				{
					$_filedata[$file] = self::get_file_info($source_dir.$file, array('settitle','setdescription','filename','basename','extension'));
	...
	public static function get_file_info($file, $returned_values = array('filename', 'server_path', 'size', 'date','extension'))
	{
		...
		foreach ($returned_values as $key)
		{
			...
			case 'basename':
				...
				break;
			case 'filename':
				$fileinfo['filename'] = pathinfo($file, PATHINFO_FILENAME);
				break;
			case 'server_path':
